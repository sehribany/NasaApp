...
class SpiritViewController: UIViewController {
    ...
    func VeriAl() {
        let url = URL(string: "https://api.nasa.gov/mars-photos/api/v1/rovers/spirit/photos?sol=1000&api_key=DEMO_KEY")
        WebService().spiritandopportunityDownload(url: url!) { (datalar) in
            if let datalar = datalar{
                DispatchQueue.main.async {
                    self.data = datalar
                    self.originalPhotos = datalar
                    NasaViewModel.shared.self.spiritandopportunityphoto = datalar
                    self.totalImages = self.data.count
                    while self.index < self.limit {
                        self.displayImages.append(self.data[self.index])
                        self.index += 1
                    }
                    self.spiritcollectionview.delegate = self
                    self.spiritcollectionview.dataSource = self
...
extension SpiritViewController : UICollectionViewDataSource, UICollectionViewDelegate{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "spirithucre", for: indexPath) as! SpiritCollectionViewCell
        let nasaViewModel = displayImages[indexPath.row]
        let imageURL = URL(string: nasaViewModel.imgSrc)
        cell.imagevieww.sd_setImage(with: imageURL)
        return cell
  }
...
extension SpiritViewController:UIPickerViewDelegate,UIPickerViewDataSource{
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        guard let url = URL(string: "https://api.nasa.gov/mars-photos/api/v1/rovers/spirit/photos?sol=1000&camera=\(cameraName)&page=0&api_key=ZBGxwNTkOJXhcEQjaXPMxok9Bq5cpBmVnjOsHEQu") else { return }
